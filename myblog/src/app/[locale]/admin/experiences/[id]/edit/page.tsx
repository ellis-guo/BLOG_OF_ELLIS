...
export default function EditExperiencePage() {
  ...
  // Fetch experience data on page load
  useEffect(() => {
    const fetchExperience = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setTitleZh(exp.titleZh);
          setTitleEn(exp.titleEn);
          setTitleFr(exp.titleFr);
          setOrganization(exp.organization);
          setLocation(exp.location || "");
          setDescriptionZh(exp.descriptionZh);
          setDescriptionEn(exp.descriptionEn);
          setDescriptionFr(exp.descriptionFr);
          setTags(exp.tags.join(", "));
          setStartDate(exp.startDate.split("T")[0]);
          setEndDate(exp.endDate ? exp.endDate.split("T")[0] : "");
          setOrder(exp.order.toString());
        } else {
